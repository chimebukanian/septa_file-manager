...
This is a complete file management application with a backend API built using Node.js/Express, PostgreSQL, and MinIO (S3-compatible storage), and a frontend built using Next.js and Tailwind CSS.
...
1. **Start Services (Postgres and MinIO)**
   ...
   docker-compose up -d
   ```
   This will start PostgreSQL on port 5432 and MinIO on ports 9000 and 9001.
...
- **Backend Stack**: Express + Sequelize (PostgreSQL). We chose Sequelize because it maps cleanly to relational databases while allowing for rapid iteration with automatic syncing and built-in soft-delete functionality (`paranoid: true`).
- **Storage**: MinIO handles S3 compatibility locally. We implemented a **Direct-to-S3 Upload** flow. The backend never receives file bytes. Instead, it generates a presigned URL, and the client pushes directly to MinIO. The client then confirms completion, and the backend verifies the file exists in storage via `HeadObject`.
- **Data Model**:
...
- **State Management**: We use React Context for Auth state, and standard React hooks (`useState`, `useEffect`) combined with SWR/Custom hooks for data fetching.
- **Upload Progress**: Server Actions don't support XHR upload progress events naturally, so we fallback to a traditional `XMLHttpRequest` in a custom hook to provide a real-time progress bar when uploading directly to the MinIO presigned URL.
...
- **Complex Permissions**: Currently, if you have a share token, you can download the file. We cut explicit "user-to-user" sharing (e.g., granting another specific email access to a folder) to focus on a polished core experience within the time limit.
...
- **Advanced Error Recovery**: While network errors are handled, we didn't implement resumable uploads since standard presigned PUT requests don't natively support resume without moving to multipart.
